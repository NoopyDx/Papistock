...
def test(request, parfum_text, number):
    ...
    for i in range(int(number)):
        ...
        if Glace.objects.filter(status = 'Todo', parfum_text = parfum).count()>0:
            ...
            new_entry = Glace(parfum_text=parfum, parfum_id=next(iter(auto_id))['id'], poids=poids/1000, date_prod=date_prod,date_dlc = date_prod+datetime.timedelta(6*365/12), no_lot=no_lot, status = 'Ordered', franchise_name = next(iter(franchise_command))['franchise_name'])
